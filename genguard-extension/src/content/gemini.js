// GenGuard Content Script for Google Gemini
...
(function () {
    ...
    const SELECTORS = {
        textArea: '.ql-editor, [contenteditable="true"][role="textbox"], .text-input-field textarea',
    ...
    // Initialize
    function init() {
        console.log('[GenGuard] Initializing Gemini monitoring...');
        checkBackendHealth();
        waitForElement(SELECTORS.textArea).then(setupMonitoring).catch(() => {
    ...
    async function checkBackendHealth() {
        try {
            ...
            console.warn('[GenGuard] ⚠ Backend not available. Start with: python main.py');
            showToast('⚠ GenGuard backend not running. Start: python main.py', 'warning');
        }
    }
    ...
    function setupMonitoring() {
        ...
        setupFileObserver();
        console.log('[GenGuard] Gemini monitoring active');
    }

    // File observer for Gemini's custom uploader
    ...
    function handleInput(event) {
        ...
        if (!isGeminiInput(target)) return;

        clearTimeout(debounceTimer);
        debounceTimer = setTimeout(() => analyzeInput(target), 300);
    }

    function isGeminiInput(el) {
        return el.matches && (el.matches(SELECTORS.textArea) || (el.closest && el.closest(SELECTORS.textArea)));
    }
    ...
    function handlePaste(event) {
        ...
        setTimeout(() => {
            const target = event.target;
            if (isGeminiInput(target)) analyzeInput(target);
        }, 100);
    }
    ...
    // Analyze image from blob URL
    async function analyzeImageFromBlob(blobUrl) {
        try {
            showToast('📄 Analyzing image with PaddleOCR...', 'info');
            const response = await fetch(blobUrl);
            const blob = await response.blob();
            await analyzeImageFile(blob);
        } catch (e) {
            console.error('[GenGuard] Blob analysis error:', e);
        }
    }
    ...
    async function analyzeTextWithBackend(text, source = 'text') {
        try {
    ...
    function showOverlay(analysis, target) {
        ...
        const container = target.closest('.input-area') || target.closest('form') || target.parentElement;
        if (!container) return;
    ...
    function showFileWarning(fileName, analysis) {
        ...
        warningDiv.innerHTML = `
          <style>
            ...
            .gfw-entities { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 12px; max-height: 100px; overflow-y: auto; }
            ...
            .gfw-btn-primary { background: linear-gradient(135deg, #ef4444, #dc2626); color: #fff; }
